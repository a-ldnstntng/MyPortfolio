...
const hexToRgb = (hex: string): { r: number; g: number; b: number } => {
  ...
  if (cleanHex.length === 3) {
    const r = Number.parseInt(cleanHex[0] + cleanHex[0], 16) || 0;
    const g = Number.parseInt(cleanHex[1] + cleanHex[1], 16) || 0;
    const b = Number.parseInt(cleanHex[2] + cleanHex[2], 16) || 0;
    return { r, g, b };
  }
...
const VerticalBarsNoise = ({
  ...
  const resizeCanvas = useCallback(() => {
    ...
    // Scale the drawing context idempotently so it never accumulates
  ...
  const animate = useCallback(() => {
    ...
    for (let i = 0; i < numLines; i++) {
      ...
      const lineAlpha = Math.max(0.2, 0.2 + mouseInfluence * 0.7);
      ...
      for (let x = 0; x < canvasWidth; x += 8) {
        ...
        if (noiseVal > threshold) {
          ...
          const intensity = Math.min(
            1,
            Math.max(0.65, 0.65 + totalInfluence * 0.35)
          );
